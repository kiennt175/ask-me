			...
			<div id="tab-top" class="col-md-8" style="margin-left: 53px">
				...
				<div class="question-tab">
					...
					@elseif (isset($searchText) && $searchText[0] != '[' && $searchText[-1] != ']')
						<a href="{{ route('questions.viewByTab', ['searchText' => $searchText ?? 'noSearching', 'tab' => 'relevance']) . '#question-search'}}"><button class="view-tab">Relevance</button></a>
					@endif

					@if (!isset($tab) || (isset($tab) && $tab == 'newest'))
                        <button class="view-tab clicked">Newest</button>
                    @else 
                        <a href="{{ route('questions.viewByTab', ['searchText' => $searchText ?? 'noSearching', 'tab' => 'newest']) . '#question-search'}}"><button class="view-tab">Newest</button></a>
                    @endif
		...
		<div class="widget widget_stats">
			...
			<div class="ul_list ul_list-icon-ok">
				<ul>
					...
					{{-- <li><i class="icon-comment"></i>Answers ( <span>{{ $totalAnswers }}</span> )</li> --}}
				</ul>
			</div>
		</div>
